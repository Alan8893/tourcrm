"""Integration tests for the initial administrator bootstrap (Issue #99 /
TH-0089, ADR-0027).

Against a real PostgreSQL database, matching
tests/integration/test_authentication_service.py's own pattern for the
pure service-layer function (app.authentication.bootstrap), plus a real
end-to-end HTTP test proving the "normal login path" contract (Issue #99
§5: after bootstrap, `POST /api/v1/auth/login` then `GET /api/v1/auth/me`
work exactly like any other active User — no special bootstrap login
flow), and a set of CLI-level tests (app.cli.bootstrap_admin) covering the
two refusal paths that only exist at the interactive-prompt layer
(password mismatch, operator cancellation).
...
def test_clean_install_creates_person_user_and_global_admin_assignment() -> None:
    email = _unique_email()
    with session_scope() as session:
        result = bootstrap_initial_administrator(session, email=email, password=_STRONG_PASSWORD)
...
def test_global_administrator_exists_reports_true_after_bootstrap() -> None:
    ...
    with session_scope() as session:
        bootstrap_initial_administrator(session, email=_unique_email(), password=_STRONG_PASSWORD)
...
def test_bootstrap_never_leaks_the_password_into_the_hash_or_audit_details() -> None:
    ...
    with session_scope() as session:
        ...
        for row in audit_rows:
            # No authenticated actor exists yet at bootstrap time.
...
# --- Repeat bootstrap (Issue #99 §1.2/§1.7) ---------------------------------
...
def test_second_bootstrap_attempt_is_rejected_without_mutation() -> None:
    ...
    with session_scope() as session:
        with pytest.raises(AdministratorAlreadyExistsError):
            bootstrap_initial_administrator(session, email=second_email, password=_STRONG_PASSWORD)
...
def test_invalid_password_is_rejected_and_creates_nothing() -> None:
    ...
    with session_scope() as session:
        with pytest.raises(WeakPasswordError):
            bootstrap_initial_administrator(session, email=email, password="short")
...
def test_duplicate_email_is_rejected_without_mutation() -> None:
    ...
    with session_scope() as session:
        with pytest.raises(EmailAlreadyRegisteredError):
            bootstrap_initial_administrator(session, email=email, password=_STRONG_PASSWORD)

    with session_scope() as session:
        assert global_administrator_exists(session) is False


# --- Missing/inconsistent canonical admin role (Issue #99 §4) --------------
...
def test_concurrent_bootstrap_attempts_produce_exactly_one_administrator() -> None:
    ...
    both call bootstrap_initial_administrator, released at the same
    moment via a Barrier — mirroring
    test_authentication_service.test_concurrent_duplicate_registration_is_rejected_not_crashed's
    own pattern. Exactly one must succeed; the other must fail with
    AdministratorAlreadyExistsError, never both succeeding, never a crash.
    """
        ...
        try:
            with session_scope() as session:
                result = bootstrap_initial_administrator(
                    session, email=email, password=_STRONG_PASSWORD
                )
            with results_lock:
            ...
            # crash here (e.g. a raw IntegrityError/deadlock escaping) is
...
# --- CLI-level refusal paths (Issue #99 §"Tests": password mismatch, -------
# --- user cancellation) -----------------------------------------------------
